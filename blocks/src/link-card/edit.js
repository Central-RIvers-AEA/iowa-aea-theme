  ...
  return (
    <>
      <InspectorControls>
        ...
        <PanelBody title={__('Settings', 'link-card')}>
          <SelectControl
            label={__('SVG Icon', 'link-card')}
          ...
          {/* Field For Text */}
          <TextControl label={__('Link Text', 'link-card')} value={attributes.linkText} onChange={(value) => setAttributes({ linkText: value })} />
          {/* Field For Link */}
      ...
      <a href='' { ...useBlockProps({ className: 'icon-card', style: { '--card-color': attributes.backgroundColor } }) } onClick={(e) => e.preventDefault()}>
        ...
        <span>
          {attributes.linkText || __('Link Card', 'link-card')}
        </span> 
      </a>
    </>
  );
